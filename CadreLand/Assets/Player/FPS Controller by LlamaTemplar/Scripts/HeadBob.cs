...
/*
 *	original code made by thatCamelCode on Unity
 *	Edited by LlamaTemplar
 *	
 */
...
public class HeadBob : MonoBehaviour
{
	...
	void Update()
	{
		if (Input.GetAxisRaw("Horizontal") != 0 || Input.GetAxisRaw("Vertical") != 0) //moving
		{
			...
			Vector3 newPosition = new Vector3(Mathf.Cos(timer) * bobAmount, camPos.y + Mathf.Abs((Mathf.Sin(timer) * bobAmount)), restPosition.z); //abs val of y for a parabolic path
